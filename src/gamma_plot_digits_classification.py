...
def digitsClassifier(data, gamma=0.001):
    ...
    return [[a_train, p_train, r_train, f1_train], [a_test, p_test, r_test, f1_test], [a_val, p_val, r_val, f1_val]]
...
for gamma in gammas:
    res = digitsClassifier(data_org, gamma=gamma)
    results_gamma.append(res)

    print(f"\ngamma: {gamma}")
    for s,r in zip(["train", "test", "val"], res):
